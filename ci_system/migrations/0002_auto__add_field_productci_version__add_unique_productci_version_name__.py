...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'CiSystem', fields ['name']
        db.delete_unique(u'ci_system_cisystem', ['name'])

        # Adding field 'ProductCi.version'
        db.add_column(u'ci_system_productci', 'version',
                      self.gf('django.db.models.fields.CharField')(default=u'', max_length=255, blank=True),
                      keep_default=False)

        # Adding unique constraint on 'ProductCi', fields ['version', 'name']
        db.create_unique(u'ci_system_productci', ['version', 'name'])


    def backwards(self, orm):
        # Removing unique constraint on 'ProductCi', fields ['version', 'name']
        db.delete_unique(u'ci_system_productci', ['version', 'name'])

        # Deleting field 'ProductCi.version'
        db.delete_column(u'ci_system_productci', 'version')

        # Adding unique constraint on 'CiSystem', fields ['name']
        db.create_unique(u'ci_system_cisystem', ['name'])
